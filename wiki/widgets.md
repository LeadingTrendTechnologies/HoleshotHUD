...
- **Show overlay users** (Settings → App, off by default) adds a mini Holeshot mark before the name on Standings / Relative / Horizontal Standings (skipped if they already have the friend mark) and a thin orange ring on map / minimap dots for other riders in this session who also have the overlay. You are not marked. Overlay does not recolor the dot. Presence is not a widget.
- **Highlight Steam friends** (Settings → App, off by default) paints map / minimap dots teal (same as the table Friend icon) and adds a Friend column on Standings / Relative (same icon before the name on Horizontal Standings) when their Steam64 is in the presence room and in your Steam friend list. Both of you must run Holeshot and have Show overlay users on. The Friend column is hidden if overlay users is off or nobody in this race is a friend. In-game names are not used. Friends without the overlay stay unmarked. The website demo turns both on and marks a few demo riders (not You).

## Shared rider colors (map, minimap, relative rows)

Default other-rider color is dark slate. **Teal** if they are a Steam friend (no overlay ring — a friend already runs Holeshot). Overlay-only users keep the slate fill and get an orange ring. **Blue** only if they are a lap ahead **and** closing from behind. **Red** only if you are a lap ahead **and** closing on them. Two (or more) laps down is still blue when they close from behind — `other_laps_ahead` prefers `gap_laps` over `num_laps` so a lapped rider whose completed-lap count sits on the race lap does not invert the leader to red. You are always orange (larger you-dot). Off in warmup (`is_warmup`) — practice lap counts are not race lapping. See `lap_rel` / `rider_dot_col` in `render.rs`. Lapping blue/red wins over friend teal.
