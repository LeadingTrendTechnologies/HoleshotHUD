...
- Draw path: `overlay/hud/src/render/` (`draw()` in `mod.rs`, one file per widget). Settings: `overlay/src/settings/`. Layout/ini: `overlay/hud/src/config.rs`. Session presets (`Practice` / `Warmup` / `Race` / `Spectate`) are four `HudLayout` slots; `session_preset()` in `race_store.rs` picks the live one. Spectate wins while `Cmd.spectating()` is set. Garage/menus hold the last slot. F8 can edit any slot on track; the HUD stays on the live preset. A live pip marks the on-track slot when you are editing another. **Copy to** duplicates the open layout onto one other slot or all four. The preset strip stays pinned under the top bar; widget panes scroll beneath it. App keys stay on `[App]`, including Settings window origin (`settings_x` / `settings_y`, virtual-screen). Ctrl-drag writes the live layout, not the F8 edit slot.
- Shared race view: `overlay/hud/src/race_store.rs` — `RaceStore::refresh` once per overlay frame, then `RaceStore::with` (re-entrant, no clone). `tick()` still returns a clone for tests and clock logs. Clock/flag sequences for a future session FSM: `overlay/hud/tests/sessions/*.jsonl`.
- Settings widget panes share `WidgetPaneSpec` / `open_widget_pane` (heading, Show on overlay, style rows, extras, snap). Standings/relative use `table_style_controls` so **Row highlight** / **Text color** / **Alternating rows** stay on those boards. App / Review / Feedback / What’s new stay custom. Review is a fourth top-bar mode ([review.md](review.md)). High-contrast settings palette is still OS-aware.
...
Default other-rider color is dark slate. **Blue** only if they are a lap ahead **and** closing from behind. **Red** only if you are a lap ahead **and** closing on them. Two (or more) laps down is still blue when they close from behind — `other_laps_ahead` prefers `gap_laps` over `num_laps` so a lapped rider whose completed-lap count sits on the race lap does not invert the leader to red. You are always orange. Off in warmup (`is_warmup`, including `session_kind` 5 when extras leak) — practice lap counts are not race lapping. See `lap_rel` / `rider_dot_col` in `render/mod.rs`.
