...
- Draw path: `overlay/hud/src/render.rs` (`draw()`). Settings: `overlay/src/settings.rs`. Layout/ini: `overlay/hud/src/config.rs`. Session presets (`Practice` / `Warmup` / `Race` / `Spectate`) are four `HudLayout` slots; `session_preset()` in `race_store.rs` picks the live one. Spectate wins while `Cmd.spectating()` is set. Garage/menus hold the last slot. F8 edits the live preset on track; the other chips stay visible but locked (dim well, no hand cursor, “switch in the garage”). A picker + Copy to all in the garage. App keys stay on `[App]`.
