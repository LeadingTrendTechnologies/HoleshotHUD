...
Gear, RPM, speed, position, and the session clock / lap counter. Race **flags** are a banner **above** this widget, not side panels. Settings subtitle: “Gear, speed, and footer stats”. Optional **Yellow flag**, **Blue flag**, and **Red flag** wrap the plaque (off by default).
...
- Flags: `dash_race_flag`, `laps_left`, `skip_last_lap_white`, `leader_finished` / `race_over_for_me`
...
A `~Lapped` tag sits beside the lap/clock text whenever `lapped` is true. Classification `gap_laps` only moves at the line, so the tag also latches the moment the lap-up leader goes past you (`LAPPED_LATCH`). Waved off a lap short still counts when the game left `gap_laps` at 0. Gated on being on track, out of warmup, and off the gate. It widens the right column, so the panel grows once when you get lapped rather than clipping. Amber (`dash_lapped_col`), a little smaller than the lap text and nudged onto its baseline.

Fixed body: gear | RPM + speed | position + lap/clock text. P1 puts a gold Font Awesome crown (`f521`, same as the map leader mark) above the place number. The plaque fills the widget rect: height scales type, extra width is column gap. Hold Ctrl and drag to resize. Default 11.1%×11.5%, bottom-centered. **Simple dash** (`dash_simple`) strips that to a compact lockup: orange skew **gear plaque** (ink-on-accent digit) plus large italic speed with a vertical `MPH`/`KPH` stack. No RPM, place, crown, footer, or rev bar. White/checkered flags still wrap the plaque; yellow/blue do too when those Dash toggles are on. The RPM/speed column is sized from the widest digits (`0`–`9`), not the live value, so gear and position do not shift as RPM changes. **P#** is Look primary color. When time penalties make live place disagree with on-track place, a trailing `*` sits on `P#` in green (ahead of on-track) or red (behind) — see [live race order](../live-order.md). **P#** and `~Lapped` use live `RaceStore` rank/gaps during a race; session clock still uses game laps. `P#` reads `standing_pos`, so it counts an on-track pass straight away instead of at the line — see [live race order](../live-order.md). It also falls back to `local_race_num` when there is no focus rider. Footer is three slots (default Engine, Air, Best); options include **Local time** (`DashField::LocalTime`, same 12h clock as standings), **Fuel** (`DashField::Fuel`, liters or US gallons from Units), **Fuel %** (`DashField::FuelPct`), **Setup** (`DashField::Setup`, loaded bike setup filename), **Gap** (`DashField::Gap`, live running gap to P1), **Interval** (live running gap to P−1), and **Gap behind** (`DashField::GapBehind`, live running gap to P+1). Optional rev bar from `local_rpm` vs `max_rpm` / `shift_rpm`. **Shift color** (`dash_shift_color`, off by default) flushes the gear faint red at the shift light or limiter — same cue as Telemetry. Simple dash flushes the orange gear plaque instead of the digit. Footer and rev stay in the ini while Simple dash is on.
...
Banner above the dash only (`draw_dash_wrap`). No striped side panels over the widget (reverted). The **Flags** widget is a separate cloth that uses the same `dash_race_flag` result for white/checkered; turning it on does not hide this wrap. **Yellow flag** (`dash_yellow`), **Blue flag** (`dash_blue`), and **Red flag** (`dash_red`) are optional Dash toggles, off by default, independent of the Flags-widget toggles. Same infer: yellow is a crash ahead and close (~50 m); blue is someone a lap up close behind (~40 m); red is someone you are coming to lap, ahead and close (~40 m). White/checkered still beat yellow, blue, and red. Blue beats red.
...
- **Lapped riders:** white and checkered still follow **your** remaining laps of the published distance. The leader taking the flag does not wave you off (`laps_left` ignores `leader_finished` / `race_over_for_me`). A 4/5 run-in is white onto your last lap; checkered is when you complete lap 5. `skip_last_lap_white` drops a first-lap crash / `session_laps` glitch that collapses remaining laps by two or more. `leader_finished` is still `session_laps <= leader_num_laps <= session_laps + 2` on a lap moto (the upper bound rejects a glitched frame where extras masquerade as a lap count), or `leader_num_laps >= overtime_base + 1 + extras` on a timed set — used for `~Lapped` and the banner total, not the flags.
- **The total shrinks to the race you run.** `effective_race_laps` is `LEADER_FIN_LOCAL_BASE + 1`, clamped to the distance: getting lapped in a 5-lap moto reads `4 / 4`, not `4 / 5`. The clamp matters — your own finish latches the base too, so a winner would otherwise read `5 / 6`. `effective_extra_laps` is `LEADER_FIN_LOCAL_BASE - OVERTIME_LOCAL_BASE`, clamped to at least one extra. That is the **banner** only. Flags and `laps_left` keep the published extras / lap count, so checkered cannot fire a lap early when the leader finishes.
...
- Footer **Gap**, **Interval**, and **Gap behind** are live-order P−1 / P+1. Same lap ticks along the track toward that rider; a live lap or more is `1L` / `-1L`. A pass switches who. Standings’ Gap / Interval columns stay the line times.
- **Shift color** starts off. A missing `dash_shift_color` key stays off. Do not flush the gear red unless that toggle is on.
- Do not put flags on the sides of the dash again.
- Do not treat leftover `10:00` or leftover `08:00` session length as a timed race when `session_laps >= 5`, or on a 4-lap moto that only ran a gate board (`LAP_MOTO_GATE`).
...
- Do not treat a live 6:00+2 (or any 5–60 min +1…+4) as a lap moto when length is 0 or still a start board after the gate (`TIMED_EXTRAS_HINT`).
...
- Do not wave the checkered because the leader finished. Flags follow your remaining laps of the published distance, even when the banner total has already shrunk.
- Do not skip last-lap white just because you are lapped. 4/5 is white onto your last lap; checkered is when you complete the distance.
...
- Do not wrap blue or red for a crashed rider. Same sticky yellow as the Flags widget (`YELLOW_HOLD_MS`).
...
- Do not drop flag wrap in Simple dash. White and checkered still sit on the compact plaque. Yellow, blue, and red do too when those Dash toggles are on.
- Do not put the P1 crown on Simple dash. That lockup has no place number.
- Do not leave the crown on P2+. It follows live `standing_pos` (on-track pass), same as `P#`.
...
- 2026-09-08 — **Gap behind** is a footer option (`DashField::GapBehind`). Live-order P+1; **Interval** stays P−1. Not Relative’s nearby-on-track rider.
- 2026-09-08 — Checkered is your finish, not the leader's. Being a lap down or the leader taking the flag does not wave you off; white/checkered still count your remaining laps of the published distance.
- 2026-09-08 — Timed motos are 5–60 min with +1…+4 extras. A 4-lap leftover `08:00` stays `1 / 4`; `60:00 +4` stays a countdown. 2–8 lap motos keep `1 / N` after a glitched 16:20 / elapsed clock.
- 2026-09-07 — Lapped riders no longer flash white before the checkered. White is the last lap you are still running; a wave-off (4/5 after being lapped, or a first-lap crash that collapses remaining laps) is checkered only, or nothing until that run-in.
- 2026-09-06 — Yellow / blue / red wrap cloth is muted ochre, slate, and brick. Same as the Flags widget.
- 2026-09-06 — **Red flag** is a Dash toggle (`dash_red`, off by default). You are a lap up and closing on a rider ahead (~40 m). Independent of `flag_red`.
- 2026-09-06 — **Yellow flag** and **Blue flag** are Dash toggles (`dash_yellow` / `dash_blue`, off by default). Same infer as the Flags widget; they wrap the plaque when on. Independent of `flag_yellow` / `flag_blue`.
- 2026-09-06 — P1 puts a gold crown above the place number. Same icon as the map leader mark. Simple dash stays gear + speed.
- 2026-09-06 — Last lap of a lap moto no longer shows `~Lapped` just because the leader took the flag. One extra completed lap is you still out on that lap, not a lap down.
- 2026-09-06 — `~Lapped` and `0/1` → `1/1` latch the moment the lap-up leader goes past you. Classification `gap_laps` waits for a line crossing, which was a lap too late.
- 2026-09-06 — Lapped on timed `0/1` starts that extra (`1/1`, `~Lapped`) and the next line is checkered. The uncounted +1 lap used to keep `laps_left == 2`, so the run-in stayed white after you were waved off.
