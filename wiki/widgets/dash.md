...
Plugin session fields are messy (warmup length leaking into race). Lots of atomics (`IN_GATE`, `SESSION_EXPIRED`, `OVERTIME_*`, `LAP_GREEN`) exist because the API does not send a clean mode enum. `session_kind` / `session_state` are in SHM (version 9). Logged: warmup `kind=5`; race 2 `kind=7` for **both** 8:00 +1 and a 4-lap moto. `state=16` is running; `state=256` is the start gate. Race 1 not dumped yet (likely `kind=6`). Lap vs timed is still `session_laps` + `session_length`, not kind. Plugin length is **`-1` until this session writes it**; a kind change drops cached length/laps so leftover `8:00` cannot stick when the new session publishes `0`. Start boards and leaked warmup extras are still real API writes, not cache.
...
- **Checkered** comes from one thing only: `laps_left == 0`, which happens when you cross the line with nothing left to run. Never from proximity to the line, and never speed-gated, so a slow roll over the line still gets waved off. It **latches** until you leave the session (`on_track == 0` clears it) and must not carry into the next warmup (0.1.4).
...
- **White** covers `laps_left == 1` (the whole final lap) plus the run-in that starts it (`laps_left == 2` and `final_lap_approach`), so there is no bare gap between white and checkered.
- **Lapped riders:** once `leader_finished` the race is over, so white shows for whatever lap you are on and the checkered comes on your next crossing (`race_over_for_me`), even a lap short. `leader_finished` is `session_laps <= leader_num_laps <= session_laps + 2` on a lap moto (the upper bound rejects a glitched frame where extras masquerade as a lap count; nobody runs more than a cool-down lap past the finish), or `leader_num_laps >= overtime_base + 1 + extras` on a timed set. `LEADER_FIN_LOCAL_BASE` latches your lap count at that moment.
...
- **Geometry is only used for the early white.** `final_lap_approach` needs the 4–80 m window (`FLAG_LINE_MIN_M` / `FLAG_LINE_M`), a mid-lap sighting (`LAP_MID_SEEN`), a closing step (`CLOSING_ON_LINE`), and a known line. Everything else is pure lap counting, so bad track data costs you the early white and can never show a wrong flag.
...
- Do not treat a live 6:00+2 (or any 5–20 min +2/+3) as a 2-lap moto when length is 0 or still a start board after the gate (`TIMED_EXTRAS_HINT`).
...
- Do not arm checkered from the S/F approach on either race type. Checkered needs a line crossing (`laps_left == 0`); the finish run-in is still white.
- Do not let track geometry decide checkered. The approach window only gates the early white, so a bad `sf_meters` or missing centerline degrades to "no early white", never to a wrong flag.
