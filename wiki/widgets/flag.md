...
Settings subtitle: “White and checkered — same timing as Dash”. Toggles: **Yellow flag** (`flag_yellow=0`), **Blue flag** (`flag_blue=0`). Old `flag_caution=1` turns both on.
...
Fresh install: `show_flag = false`, `flag_yellow = false`, `flag_blue = false`. Default ~10.7%×1.9% (the size we settled on in-game), top-center. Hold Ctrl and drag to place. Panel opacity tints the cloth (default 100). Caption white uses the same 5% edge fade as the Dash wrap banners, with extra side pad so it is not tight on the glyphs. The white band is shorter than the plaque so cloth shows above and below.
