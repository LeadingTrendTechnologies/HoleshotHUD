...
- Widgets: Standings, Relative, Horizontal Standings, Map, Minimap, Radar, Dash, Systems. Sectors is labs-only until unlocked.
- Fresh install: every **Show on overlay** toggle starts off. Nothing draws until the rider turns it on.
...
- Auto-update from GitHub releases; in-app feedback (F8 → App) posts private gists.
